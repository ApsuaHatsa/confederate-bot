...
async def start_discord_bot():
    try:
        ...
        if "PrivilegedIntentsRequired" in type(e).__name__ or "privileged intents" in str(e).lower():
            logger.warning("PrivilegedIntentsRequired: Disabling presences/members intents and retrying...")
            discord_bot._intents.presences = False
            discord_bot._intents.members = False
            await discord_bot.start(DISCORD_TOKEN)
        else:
            raise e
...
async def start_http_server():
    """Lightweight HTTP server for Render.com health checks and keep-alive."""
    port = int(os.environ.get("PORT", 10000))
    async def handle_health(request):
        return web.Response(text="Confederate Bot OK")
    app = web.Application()
