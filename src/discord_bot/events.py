...
async def on_message(message: discord.Message):
    ...
    if message.content and message.content.strip().startswith("/atb"):
        lang = get_chat_lang(chat_id) or "en"
        parts = message.content.strip().split()
        if len(parts) >= 2:
            raw = parts[1].strip()
            if db.chat_exists(chat_id):
                await message.channel.send(localized("atb_already_attached", lang))
                return
            if raw.lower() == "new":
                ...
                try:
                    ...
                    await message.channel.send(localized("atb_invalid_id", lang))
                    return
                db.attach_chat("discord", chat_id, bridge_id)
                reply_key = "atb_attached"
            try:
                await message.channel.send(localized_bot_joined(lang))
            except Exception:
                pass
            await message.channel.send(localized(reply_key, lang, bridge_id=bridge_id))
            return
